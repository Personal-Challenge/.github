...
| `personal-todo-deploy`   | Orquestación   | `docker-compose.yml`, `setup.sh`, `Makefile`, variables de entorno, seed de datos y documentación de arquitectura.      |

> Nota: este repositorio de deploy no versiona el código fuente de `backend/` ni `frontend/`. Esas carpetas son generadas localmente por `setup.sh` al clonar o actualizar los repositorios de aplicación.
...
El frontend consume la API REST del backend. La configuración concreta de URL y endpoint se documenta en el README del repositorio de deploy.
